...
class LoginTasksTest extends TestCase
{
    ...
    {
        Horde_LoginTasks_Stub_First::$executed = [];
        $tasks = $this->_getLoginTasks(
            ['Horde_LoginTasks_Stub_First']
        );
        $tasks->runTasks();
        $this->assertEquals(
            ['Horde_LoginTasks_Stub_First'],
    ...
    {
        Horde_LoginTasks_Stub_Once::$executed = [];
        $tasks = $this->_getLoginTasks(
            ['Horde_LoginTasks_Stub_Once']
        );
        $tasks->runTasks();
        $this->assertEquals(
            ['Horde_LoginTasks_Stub_Once'],
    ...
    public function testAfterConfirmationOfADisplayedTaskTheUserIsRedirectedToTheUrlStoredBeforeDisplaying()
    {
        ...
        $this->assertStringContainsString(
            'URL',
            (string) $tasks->runTasks(['user_confirmed' => true])
        );
        $this->assertNull(
            $tasks->runTasks()
        );
        ...
        $this->assertEquals(
            'redirect',
            $tasks->runTasks(['user_confirmed' => true])
        );
    }
    ...
    public function testConfirmSeriesDisplay()
    {
        ...
        $this->assertStringContainsString(
            'URL',
            (string) $tasks->runTasks(['url' => 'redirect'])
        );
        $this->assertEquals(
            [
            ],
    ...
    private function _getLoginTasks(array $tasks = [], $last_run = false)
    {
        if ($last_run && !is_bool($last_run)) {
            $last_run = ['test' => $last_run->timestamp()];
        }
